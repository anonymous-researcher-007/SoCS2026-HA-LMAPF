...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Settings match compare_global_study_agents.py:\n"
            "  Maps:   random, warehouse_small (small), warehouse_large, den520d (large)\n"
            "  Small:  agents=[25,50,75,100]  humans=50\n"
            "  Large:  agents=[100,200,300,400]  humans=200\n"
            "  Seeds=15, steps=2000, fov=5, safety_radius=1, budget=5s\n"
            "\n"
    ...
    out = make_output_dir("horizon", run_label=args.label, base_dir=args.output)
